...
export default function Project() {
    ...
    title,date,
    place,description,projectType,link,tags
                    ...
                    {projectData && projectData.map((project, index) => (
                        <article className=" relative rounded-lg shadow-xl bg-white p-16">
                            <h3 className="text-gray-800 text-3xl font-bold mb-2 hover:text-red-700 ">
                               ...
                               className="text-blue-900" >{project.title}</a>
                            </h3>
                            <div className="text-gray-500 text-xs space-x-4">
                                <span><strong className="font-bold">Finished on</strong>:{" "}{new Date(project.date).toLocaleDateString()}</span>
                                <span><strong className="font-bold">Location</strong>:{" "}{project.place}</span>
                                ...
                                >
                                    View The Project{" "} <span role="img" aria-label="right-pointer">
                                        👍
                                   </span>
                                </a>
                            </div>
